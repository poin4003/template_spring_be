...
import com.app.core.exception.category.AppSecurityException;
import com.app.core.exception.category.BusinessException;
import com.app.core.exception.category.UnknownException;
import com.app.core.exception.category.ValidationException;
import com.app.core.mq.handler.MessageHandler;
import com.app.core.mq.handler.MessageHandlerRegistry;
...
public class MqlistenerServiceImpl implements MqListenerService {
    ...
    @Override
    public void registerMqConsumer(MqConsumerSetup input) {
        if (input == null) {
            log.warn("Skip registerMqConsumer: input is null");
            return;
        }

        Objects.requireNonNull(input.getEndpointId(), "endpointId is required");
        Objects.requireNonNull(input.getHandlerKey(), "handlerKey is required");
        Objects.requireNonNull(input.getSourceName(), "sourceName is required");
        Objects.requireNonNull(input.getConsumerGroup(), "consumerGroup is required");

        String listenerId = "dynamic-mq-" + input.getEndpointId();

        MessageHandler<?> handler = Objects.requireNonNull(
            handlerRegistry.get(input.getHandlerKey()),
            "MessageHandler not found for key: " + input.getHandlerKey()
        );
        ...
        endpoint.setGroupId(input.getConsumerGroup());
        endpoint.setTopics(input.getSourceName());
    ...
    buildContainerFactory(MqConsumerSetup input) {
        ...
        factory.setCommonErrorHandler(
            Objects.requireNonNull(createErrorHandler(input), "ErrorHandler must not be null")
        );
    ...
    private DefaultErrorHandler createErrorHandler(MqConsumerSetup input) {
        ...
        ConsumerRecordRecoverer recoverer = createRecoverer(input);
    ...
    private ConsumerRecordRecoverer createRecoverer(MqConsumerSetup input) {
        boolean dlqEnabled = Boolean.TRUE.equals(input.getEnableDlq());

        if (!dlqEnabled) {
            return (record, ex) -> log.error(
                "[Recoverer] Consumer [{}] exhauted retries. Message discarded. Error: {}",
                input.getEndpointId(), ex.getMessage()
            );
        }

        String dlqName = (input.getDlqName() != null && !input.getDlqName().isEmpty())
            ? input.getDlqName()
            : input.getSourceName() + ".DLQ";

        log.info("Consumer [{}] DLQ enabled -> Target: {}", input.getEndpointId());
